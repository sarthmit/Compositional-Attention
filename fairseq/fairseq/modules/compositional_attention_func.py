...
def get_mask(num_heads, num_values):
    pass

@with_incremental_state
class CompositionalAttention_func(nn.Module):
    ...
    def __init__(
        ...
        if self.qk_rule:
            self.value_k = nn.Parameter(torch.zeros(1, 1, 1, self.num_rules, self.selection_dim))
            if self.q_compose:
            ...
            if self.nonlinear:
                self.score_network1 = quant_noise(
                    nn.Linear(2 * self.selection_dim, self.selection_dim, bias=bias), q_noise, qn_block_size
                )
                ...
                self.score_network = quant_noise(
                    nn.Linear(2 * self.selection_dim, 1, bias=bias), q_noise, qn_block_size
                )
    ...
    def reset_parameters(self):
        ...
        if self.qk_rule:
            nn.init.xavier_uniform_(self.value_k, gain=1 / math.sqrt(2))
            nn.init.xavier_uniform_(self.value_q.weight, gain=1 / math.sqrt(2))
        else:
            nn.init.xavier_uniform_(self.value_k)
    ...
    def forward(
        ...
        if saved_state is not None:
            ...
            key_padding_mask = CompositionalAttention_func._append_prev_key_padding_mask(
        ...
        if self.qk_rule:
            ...
            v_k = self.value_k.expand(tgt_len, bsz, self.num_heads, -1, -1).transpose(4,3).contiguous()
            v_score = torch.matmul(v_q, v_k).view(tgt_len, bsz, self.num_heads, self.num_rules, 1)
        else:
            v_q = v_q.repeat(1, 1, 1, self.num_rules, 1)
            v_k = self.value_k.expand(tgt_len, bsz, self.num_heads, -1, -1)
            v_in = torch.cat([v_q, v_k], dim=-1)
            if self.nonlinear:
